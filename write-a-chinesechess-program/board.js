...
function Board(container, images) {
  ...
  this.computer = -1;			// this.computer = 0 表示电脑执黑 this.computer = 1表示电脑执红
  ...
  for (var sq = 0; sq < 256; sq ++) {
	...
	// 3.每个棋盘区域都会绑定点击事件 参数sq表示了具体点击的区域 用到了闭包
	...
	// 5.将img标签存储到imgSquares数组中，方便后续对该区域进行操作 比如显示不同的棋子图片
	this.imgSquares.push(img);
  }
  
  // 电脑思考中的图片thinking.gif
...
Board.prototype.clickSquare = function(sq_) {
  ...
  var sq = this.flipped(sq_);		// 点击的位置  如果是电脑执红 位置是被翻转的  再执行一遍flipped  位置就被翻转回来
  ...
  if ((pc & SIDE_TAG(this.pos.sdPlayer)) != 0) {
    ...
    // 点击的不是己方棋子 对方棋子或者无子的位置  但有子选中 一定是自己的棋子  那么就执行这个走法
	this.addMove(MOVE(this.sqSelected, sq), false);
  }
}

// 显示sq位置的棋子图片 如果该位置没有棋子 则显示一张透明的图片 如果selected为true 则要显示棋子选中时的边框
...
Board.prototype.restart = function(fen) {
  ...
  this.response();				// 如果电脑执红先走 会自动先走步棋
}
