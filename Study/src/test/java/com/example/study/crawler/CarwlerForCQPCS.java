...
import com.example.study.mapper.dqjc.PoliceSubstationInfoNewMapper;
import com.example.study.mapper.dqjc.SyncDwMapper;
import com.example.study.mapper.dqjc.SysDictMapper;
...
public class CarwlerForCQPCS {
    ...
    @Resource
    private PoliceSubstationInfoNewMapper policeSubstationInfoNewMapper;
    ...
    private String NEW_PC_STATION_LIST_KEY = "dqjc:crawler:pc:11:13:station:list";
    ...
    public void getPcStation(){
        ...
        for (SysDictVO sdv : dictVOList){
            dictMap.put(sdv.getDictName(), sdv.getDictCode());
        }
        ...
        for (String  str : table.rowKeySet()){
            ...
            for (Map.Entry<String, String> entry : map.entrySet()){
                PoliceSubstationInfo psi = new PoliceSubstationInfo();
                psi.setPcsname(entry.getKey());
                psi.setRegion(str);
                psi.setAreacode(dictMap.get(str));
                System.out.println(str + "------" + entry.getKey());
                policeSubstationInfoNewMapper.insertSelective(psi);
            }
